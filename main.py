...
def parse_training_args(parser):
    ...
    training.add_argument('--cudnn-benchmark',
                          default=True,
...
def train():
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            writer.add_scalar('Training/false_negtive_rate', false_negtive_rate,
                              iteration)
                ...
                if (hp.in_class == 1) and (hp.out_class == 1):
                    ...
                    source_image.save(
                        os.path.join(args.output_dir,
                                     f"step-{epoch:04d}-source" + hp.save_arch))
                    ...
                    output_image_artery = torchio.ScalarImage(tensor=outputs[0],
                                                              affine=affine)
                    ...
                    output_image_trachea.save(
                        os.path.join(
                            args.output_dir,
                            f"step-{epoch:04d}-predict_trachea" + hp.save_arch))

                    label_image_vein = torchio.ScalarImage(tensor=y[3],
                                                           affine=affine)
                    label_image_vein.save(
                        os.path.join(args.output_dir,
                                     f"step-{epoch:04d}-gt_vein" +
                                     hp.save_arch))
        ...
        if (hp.in_class == 1) and (hp.out_class == 1):
            ...
            label_image.save(
                os.path.join(output_dir_test,
                             f"{i:04d}-result_float" + hp.save_arch))
            ...
            output_image.save(
                os.path.join(output_dir_test,
                             f"{i:04d}-result_int" + hp.save_arch))
        else:
            ...
            output_image_artery_float.save(
                os.path.join(output_dir_test,
                             f"{i:04d}-result_float_artery" + hp.save_arch))
